...
function parseHTML(html) {
  ...
  function(response) {
      ...
      if (joz == undefined) {
       ...
       {},
       function(response){console.log(response.status);$scope.matn="کد خطا: 3 "+response.status;if (!$scope.$$phase) $scope.$apply();
         $scope.matn="حله انداختیمش بیرون";if (!$scope.$$phase) $scope.$apply();
       },
       function(response){console.log(response.status);$scope.matn="کد خطا: 2 "+response.status;if (!$scope.$$phase) $scope.$apply();})   
     },
     function(response) {console.log(response.status);$scope.matn="نتونستم به سرور متصل شم.کد خطا:  "+response.status;if (!$scope.$$phase) $scope.$apply();}
     );
  },function(response){
    $scope.matn="نتونستم به سرور متصل شم.کد خطا:"+response.status;if (!$scope.$$phase) $scope.$apply();
  })
